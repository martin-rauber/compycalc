...
SS_for_a<-function(par) {
  ...
  alfa.nv<-exp(log(alfa0)*exp(5-(1/T))) # non volatile
  alfa.v<-exp(log(alfa0)*exp(5-(b/T)))  # volatile
...
up_par<-c(rep(5,n),rep(0.25,n))             #vector of upper bound a's and T's
...
for (samples in c(1:n)) {
  ...
  up_par<-c(rep(5,n_taken),rep(0.25,n_taken))             #vector of upper bound a's and T's
...
alfa.nv.all<-alfa0^(exp(5-(1/T))) # non volatile
alfa.v.all<-alfa0^(exp(5-(b_fixed/T)))  # volatile
...
alfav_err_T<-b_fixed*log(alfa0)*exp(5-(b_fixed/T))*(1/T^2)*alfa0^(exp(5-(b_fixed/T)))
alfanv_err_T<-log(alfa0)*exp(5-(1/T))*(1/T^2)*alfa0^(exp(5-(1/T)))
